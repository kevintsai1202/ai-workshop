...
# 手機板響應式驗證（iPhone SE / iPhone 13 / iPad Mini / Desktop 共 4 種視口）
# 截圖輸出於 data/mobile-verify/，含 home / day1 / sidebar-open / toolbox 多角度
...
### Sidebar 開合：桌機 / 手機是兩套 class（不能合併）

Sidebar 切換有**兩種互斥**的模式，由 `matchMedia('(max-width: 768px)')` 切換：

- **桌機**：sidebar 預設可見，toggle `.app.sidebar-closed` 收合（`transform: translateX(-100%)` + `.main { margin-left: 0 }`）
- **手機（≤768px）**：sidebar 預設隱藏（CSS 把 `.sidebar` 直接 `transform: translateX(-100%)`），toggle `.app.sidebar-open` 拉出 overlay。`.sidebar-backdrop` 元素提供半透明遮罩、點擊關閉

**不要合併成單一 class**：曾試圖用「`.sidebar-closed` 在桌機等於收合、在手機等於展開」這種反相邏輯，但由於兩個方向的預設狀態相反（桌機預設展開、手機預設收合），同一個 class 會在兩個 viewport 下表達相反語意，CSS 與 JS 都極難維護。視窗 resize 跨越 768px 斷點時，JS 會清除兩個 class 並回到該 viewport 的預設狀態。

### 自訂縮放（zoom +/−）只在桌機生效

`applyZoom()` 偵測 `matchMedia('(max-width: 768px)')`：手機上呼叫 `removeProperty('--content-zoom')` 清掉 inline style，讓 `@media` 中的 `:root { --content-zoom: 1 }` 接管（因為 inline style 的 specificity 高於 media query 規則，不清掉的話手機會被桌機 1.35 的偏好覆蓋）。手機上 `.nav-zoom` 也直接 `display: none`，避免使用者誤點。
