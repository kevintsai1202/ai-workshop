...
├── course-data.js    ← 課程資料（4 天大綱、17 條 RTFC 提示詞、34 個任務、10 份素材、10 題測驗）
└── README.md         ← 本說明
```
...
| **互動測驗**    | 結訓測驗 10 題（含 1 題多選），「送出 / 即時批改」會顯示分數、標記正解與錯誤、給出對應章節指引。 |
...
- 出席時數達 80% 以上
- 完成前後測考試（後測即為本網頁的「結訓測驗」10 題）
- 達 70 分（10 題答對 7 題以上）為結訓標準
